...
struct BilboTextHtmlParserNode
{
	...
	inline bool allowedInContext(int parentId) const
	{
		switch (id) {
			...
			case Html_tr: return (parentId == Html_table || parentId == Html_thead || parentId == Html_tbody || parentId == Html_tfoot);
			case Html_th:
...
class BilboTextHtmlParser
{
	public:
		...
		inline const BilboTextHtmlParserNode &at(int i) const { return nodes.at(i); }
		inline BilboTextHtmlParserNode &operator[](int i) { return nodes[i]; }
